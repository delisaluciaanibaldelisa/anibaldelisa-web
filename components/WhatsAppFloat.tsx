...
// según la ruta. data-wa-num="chapa" deriva al WhatsApp de Chapa y Pintura.

const NUM_GENERAL = "59899680555";
...
export default function WhatsAppFloat() {
  ...
  const defaultMsg =
    porRuta.find(([p]) => pathname.startsWith(p) && p !== "/")?.[1] ??
    "Hola! Quiero hacer una consulta sobre el taller";

  const [msg, setMsg] = useState(defaultMsg);
  const [num, setNum] = useState(NUM_GENERAL);

  useEffect(() => {
    setMsg(defaultMsg);
    setNum(NUM_GENERAL);

    const targets = document.querySelectorAll<HTMLElement>("[data-wa-msg]");
    if (targets.length === 0) return;

    const observer = new IntersectionObserver(
      (entries) => {
        // Toma la sección visible más cercana al centro de la pantalla.
        const visible = entries
          .filter((e) => e.isIntersecting)
          .sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0];
        if (visible) {
          const el = visible.target as HTMLElement;
          setMsg(el.dataset.waMsg || defaultMsg);
          setNum(el.dataset.waNum === "chapa" ? NUM_CHAPA : NUM_GENERAL);
        }
      },
      { threshold: [0.35] },
    );
